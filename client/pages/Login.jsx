...
export default function Login() {
  ...
  const handleSendOTP = () => {
    if (loginData.phoneNumber) {
      setIsOTPSent(true);
      console.log("OTP sent to:", loginData.phoneNumber);
      ...
      <header className="w-full border-b border-[#E5E8EB] bg-white">
        <div className="flex justify-between items-center px-10 py-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/" className="text-[#0D141C] font-bold text-lg leading-6 hover:text-[#4799EB] transition-colors">
              ElderConnect
            </Link>
          </div>
      ...
      <main className="py-5 px-4 md:px-8 lg:px-40">
        <div className="max-w-6xl mx-auto">
          <div className="max-w-lg mx-auto">
            {/* User Type Tabs */}
            <div className="border-b border-[#D1DBE8] mb-3">
              <div className="flex px-4">
                {tabs.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`px-0 py-4 border-b-3 font-bold text-sm leading-5 transition-colors ${
                      activeTab === tab.id
                        ? "text-[#0D141C] border-[#0D141C]"
                        : "text-[#4F7396] border-transparent hover:text-[#0D141C]"
                    }`}
            ...
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <div className="px-4 py-3">
                <Input
                  ...
                  className="h-14 bg-[#E8EDF2] border-0 text-base placeholder:text-[#4F7396] focus-visible:ring-1 focus-visible:ring-[#4799EB]"
                />
              </div>
              ...
              <div className="px-4 py-3">
                <Button 
                  type="submit"
                  className="w-full h-12 bg-[#4799EB] hover:bg-[#3a7bc2] text-[#0D141C] font-bold text-base rounded-lg border-0"
                >
                  Login
                </Button>
              </div>
            ...
            <div className="px-4 py-1">
              <p className="text-[#4F7396] text-sm text-center leading-5">
                Or login with OTP
              </p>
            </div>
            ...
            <div className="px-4 py-1">
              <p className="text-center">
                <a 
                  ...
                  className="text-[#4F7396] text-sm leading-5 hover:text-[#4799EB] transition-colors"
                >
                  Forgot Password?
                </a>
              </p>
